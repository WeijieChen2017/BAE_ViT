...
_C.MI.MAX_SAVE = 1
_C.MI.CRITERION = 'l1'
_C.MI.SAVE_PREDS = False
_C.MI.DEBUG = False
_C.MI.IS_PRETRAINED = False
_C.MI.PRETRAINED_PATH = ''
_C.MI.IMAGE_MEAN = None
_C.MI.IMAGE_STD = None
_C.MI.LABEL_SEPERATOR = '|'
_C.MI.REG_RESUME = ''
_C.MI.IS_GENDER_INFO = False
_C.MI.REG_DROP = 0.5
_C.MI.PAD_TO_SIZE = False
_C.MI.IS_MEDICAL_TRANSFORMS = False
_C.MI.MULTI_CROP = False
_C.MI.MULTI_CROP_NUM = 10
_C.MI.MULTI_MODEL = False
_C.MI.CKPTS = None
_C.MI.FREEZE_IMG_BRANCH = False
_C.MI.GENDER_DIM = 32
_C.MI.ALPHA = 10.0
_C.MI.GENDER_FILTER = None
_C.MI.LAYER_LR_DECAY = 1.0
_C.MI.GEN_HEATMAP = False
_C.MI.TEST_ONLY = False
...
def update_config(config, args):
    ...
    ## Medical Imaging
    config.MI.MAX_SAVE = args.max_save
    config.MI.CRITERION = args.criterion
    if args.save_preds:
        config.MI.SAVE_PREDS = True
    if args.debug:
        config.MI.DEBUG = True
    if args.is_pretrained:
        config.MI.IS_PRETRAINED = True
    config.SEED = args.random_seed
    config.MI.PRETRAINED_PATH = args.pretrained_path
    config.MI.REG_RESUME = args.reg_resume
    if args.epochs > 0:
        config.TRAIN.EPOCHS = args.epochs
    if args.multicrop_test:
        config.MI.MULTI_CROP = True
    if args.multi_model:
        config.MI.MULTI_MODEL = True
    if args.model_ckpts:
        config.MI.CKPTS = args.model_ckpts
    if args.freeze_image_branch:
        config.MI.FREEZE_IMG_BRANCH = args.freeze_image_branch
    if args.gender_dim > 0:
        config.MI.GENDER_DIM = args.gender_dim
    if args.alpha >= 0:
        config.MI.ALPHA = args.alpha
    if args.gender_filter >= 0:
        config.MI.GENDER_FILTER = args.gender_filter
    if args.base_lr > 0:
        config.TRAIN.BASE_LR = args.base_lr
    if args.layer_lr_decay > 0:
        config.MI.LAYER_LR_DECAY = args.layer_lr_decay
    if args.gen_heatmap:
        config.MI.GEN_HEATMAP = True
    if args.no_test_crop:
        config.TEST.CROP = False
    if args.test_pad:
        config.MI.PAD_TO_SIZE = True
    if args.warmup >= 0:
        config.TRAIN.WARMUP_EPOCHS = args.warmup
    if args.test_only:
        config.MI.TEST_ONLY = True
